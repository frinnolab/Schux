        ...
        <div class="row">
            <div class="col-sm-10">
                <form class="form-horizontal" role="form" method="post">
                    ...
                    <?php 
                        if(form_error('combinedSubjectID')) 
                            echo "<div class='form-group has-error' >";
                        else     
                            echo "<div class='form-group' >";
                    ?>
                        <label for="combinedSubjectID" class="col-sm-2 control-label">
                            <?=$this->lang->line("routine_combined_subject")?>
                        </label>
                        <div class="col-sm-6">
                            <?php
                                $arraysubject = array('0' => $this->lang->line("routine_subject_combined_select"));
                                if(count($subjects)) {
                                    foreach ($subjects as $subject) {
                                        $arraysubject[$subject->subjectID] = $subject->subject;
                                    }
                                }
                                echo form_dropdown("combinedSubjectID", $arraysubject, set_value("combinedSubjectID"), "id='combinedSubjectID' class='form-control select2'");
                            ?>
                        </div>
                        <span class="col-sm-4 control-label">
                            <?php echo form_error('combinedSubjectID'); ?>
                        </span>
                    </div>

                    <?php 
                        if(form_error('combinedTeacherID')) 
                            echo "<div class='form-group has-error' >";
                        else     
                            echo "<div class='form-group' >";
                    ?>
                        <label for="combinedTeacherID" class="col-sm-2 control-label">
                            <?=$this->lang->line("routine_combined_teacher")?>
                        </label>
                        <div class="col-sm-6">
                             <?php
                                $arrayteacher[0] = $this->lang->line('routine_select_combined_teacher');
                                if(count($teachers)) {
                                    foreach ($teachers as $key => $teacher) {
                                        $arrayteacher[$teacher->teacherID] = $teacher->name;
                                    }
                                }
                                echo form_dropdown("combinedTeacherID", $arrayteacher, set_value("combinedTeacherID"), "id='combinedTeacherID' class='form-control select2'");
                            ?>
                        </div>
                        <span class="col-sm-4 control-label">
                            <?php echo form_error('combinedTeacherID'); ?>
                        </span>
                    </div>
                    ...
                    <?php 
                        if(form_error('combinedRoom')) 
                            echo "<div class='form-group has-error'>";
                        else     
                            echo "<div class='form-group'>";
                    ?>
                        <label for="combinedRoom" class="col-sm-2 control-label">
                            <?=$this->lang->line("routine_combined_room")?>
                        </label>
                        <div class="col-sm-6">
                            <input type="text" class="form-control" id="combinedRoom" name="combinedRoom" value="<?=set_value('combinedRoom')?>" >
                        </div>
                        <span class="col-sm-4 control-label">
                            <?php echo form_error('combinedRoom'); ?>
                        </span>
                    </div>
...
$(document).on('change', '#combinedSubjectID', function() {
    var combinedSubjectID = $(this).val();
    if(combinedSubjectID == 0) {
        $('#combinedTeacherID').val(0);
    } else {
        $.ajax({
            type: 'POST',
            url: "<?=base_url('routine/teachercall')?>",
            data: { 'subjectID' : combinedSubjectID},
            dataType: "html",
            success: function(data) {
               $('#combinedTeacherID').html(data);
            }
        });
    }
});
