                ...
                <?php if($this->session->userdata('usertypeID') != 3) { ?> 
                    <h5 class="page-header">
                        <div class="col-lg-2 col-sm-2 col-md-2 col-xs-12 pull-right drop-marg">
                            <?php
                                $array = array("0" => $this->lang->line("lmember_select_class"), "-1" => $this->lang->line("lmember_teacher"));
                                if(count($classes)) {
                                            ...
                                            <?php if(count($students)) {$i = 1; foreach($students as $student) { ?>
                                                <tr>
                                                    ...
                                                    <td data-title="<?=$this->lang->line('lmember_name')?>">
                                                        <?php echo $student->srname ?: $student->name; ?>
                                                    </td>

                                                    <td data-title="<?=$this->lang->line('lmember_roll')?>">
                                                        <?php echo $student->srroll ?: 'Teacher - '.$student->teacherID; ?>
                                                    </td>
                                                    ...
                                                    <td data-title="<?=$this->lang->line('action')?>">
                                                        <?php
                                                            $memberID = $student->studentID?:$student->teacherID;
                                                            $memberStatus = $student->studentID ? 'student' : 'teacher';
                                                            if($student->library == 0) {
                                                                if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
                                                                    echo btn_add('lmember/add/'.$memberID."/".$set."?member=".$memberStatus, $this->lang->line('lmember'));
                                                                }
                                                            } else {
                                                                echo btn_view('lmember/view/'.$memberID."/".$set."?member=".$memberStatus, $this->lang->line('view')). " ";
                                                                if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
                                                                    echo btn_edit('lmember/edit/'.$memberID."/".$set."?member=".$memberStatus, $this->lang->line('edit')). " ";
                                                                    echo btn_delete('lmember/delete/'.$memberID."/".$set."?member=".$memberStatus, $this->lang->line('delete'));
                                                                }
                                                            }
                                                        ?>
                                                    </td>
